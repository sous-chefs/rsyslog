...
- `node['rsyslog']['custom_remote']` - Array of hashes for configuring custom remote server targets used by the remote_logging recipe
...
## Recipes
### default
Installs the rsyslog package, manages the rsyslog service and sets up basic configuration for local logging.

### remote_logging
Includes the default recipe and also sets up remote logging based on configuration in the `node['rsyslog']['custom_remote']` attribute.

#### custom_remote options:
